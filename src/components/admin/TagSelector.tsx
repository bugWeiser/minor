...
export default function TagSelector({ selectedTags, onChange }: TagSelectorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div 
        onClick={toggleAll}
        className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all cursor-pointer ${hasAll ? 'bg-accent/10 border-accent shadow-lg shadow-accent/10' : 'bg-white border-border-subtle hover:border-border-strong group'}`}
      >
        <div className="flex items-center gap-4">
           <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${hasAll ? 'bg-accent text-charcoal shadow-md' : 'bg-bg-card-secondary text-text-muted group-hover:bg-accent group-hover:text-charcoal'}`}>
        ...
        <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${hasAll ? 'bg-charcoal border-charcoal text-white' : 'border-border-strong group-hover:border-charcoal'}`}>
           {hasAll && <HiOutlineCheck className="w-4 h-4" />}
        </div>
      </div>
      ...
      <div className="bg-white border border-border-subtle rounded-3xl overflow-hidden shadow-sm shadow-black/5 ring-1 ring-black/5">
        {/* Header Grid */}
        <div className="grid bg-bg-card-secondary border-b border-border-subtle"
        ...
        {DEPARTMENTS.map((dept) => (
          <div
            key={dept}
            className="grid border-b last:border-b-0 border-border-subtle border-dashed transition-colors hover:bg-bg-hover/30"
            style={{ gridTemplateColumns: '100px repeat(4, 1fr)' }}
          >
